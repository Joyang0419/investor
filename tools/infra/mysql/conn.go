...
func SetupConn(config Config, logger logger.Interface) *gorm.DB {
	// dataSourceName
	dsn := fmt.Sprintf(
	...
	if err != nil {
		panic(fmt.Sprintf("[SetupConn]gorm.Open err: %v", err))
	}

	sqlDB, err := db.DB()
	if err != nil {
		panic(fmt.Sprintf("[SetupConn]db.DB err: %v", err))
	}
